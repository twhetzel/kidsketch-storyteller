...
async def create_story_beat(session_id: str, user_instruction: Optional[str] = None):
    ...
    # 3. Set beat image: use inline image from Gemini when present, else fall back to Imagen
    local_img_path = f"/tmp/{beat.id}.png"
    if inline_image_bytes:
        # Use the image generated by Gemini in the interleaved response
        ...
        # Fallback: no inline image (e.g. model didn't return one) — call ImageGenService
